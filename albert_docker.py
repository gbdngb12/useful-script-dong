        ...
        try:
            for container in self.client.containers.list(all=True):
                if query.string in container.name:
                    ...
                    actions.extend([
                        Action("attach-bash", "Attach bash",
                               lambda c=container.id: runTerminal("docker exec -it %s bash" % c)),
                        Action("attach-zsh", "Attach zsh",
                               lambda c=container.id: runTerminal("docker exec -it %s zsh" % c)),
